...
public class AntichemRenderer extends EntityRenderer<AntichemEntity> {
	...
	public AntichemRenderer(EntityRendererProvider.Context context) {
		super(context);
		model = new Modelmissile(context.bakeLayer(Modelmissile.LAYER_LOCATION));
	}

	@Override
	public void render(AntichemEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
	...
	@Override
	public ResourceLocation getTextureLocation(AntichemEntity entity) {
		return texture;
	}
}
